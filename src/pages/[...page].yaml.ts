 ...
 * [...page].yaml.ts
 *
 * This is a dynamic API route that generates YAML data for all content pages.
 * It uses Astro's spread parameter syntax ([...page]) to capture all URL paths
 * and return structured YAML data based on the site's data configuration.
 * This enables programmatic access to page content for applications, APIs,
 * and other services that need structured data in YAML format.
 *
 * Features:
 ...
 * 5. GET() function returns YAML response with page data
 *
 ...
 * - Enables pre-rendering of YAML data for better performance
 *
 * Prerendering:
 * - prerender = true: Enables static site generation for better performance
 * - YAML data is generated at build time for fast API responses
 *
 * Response Handling:
 * - Returns YAML serialized page data using js-yaml library
 ...
 *
 * Usage Context:
 * - Dynamic API endpoint for page data in YAML format
 * - Programmatic access to site content in YAML
 ...
 * which page data endpoints should be pre-rendered as static YAML files.
 ...
 *
 * This function is called for each dynamic route to generate the YAML
 * response containing the page data. It uses the page parameter from
 * the URL to load the appropriate content, serializes it to YAML format,
 * and returns it with proper headers.
 *
 * @param params - Object containing route parameters
 * @param params.page - The page slug extracted from the URL
 * @returns Response object with YAML data and appropriate headers
 */
